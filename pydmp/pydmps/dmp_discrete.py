...
class DMPs_discrete(DMPs):
    ...
    def __init__(self, **kwargs):
        ...
        self.variance = np.ones(self.n_bfs) * self.n_bfs**1.5 / self.center/ self.canonical_system.gain_a
    ...
    def gen_centers(self):
        ...
        '''x_track = self.canonical_system.discrete_rollout()
        t = np.arange(len(x_track))*self.dt
        # choose the points in time we'd like centers to be at
        c_des = np.linspace(0, self.canonical_system.run_time, self.n_bfs)
        self.center= np.zeros(len(c_des))
        for ii, point in enumerate(c_des):
            diff = abs(t - point)
            self.center[ii] = x_track[np.where(diff == min(diff))[0][0]]'''

        # desired activations throughout time
        des_c = np.linspace(0, self.canonical_system.run_time, self.n_bfs)
        

        self.center= np.ones(len(des_c))
        for n in range(len(des_c)):
            # finding x for desired times t
            self.center[n] = np.exp(-self.canonical_system.gain_a * des_c[n])
    ...
    def gen_goal(self, y_des):
        ...
        return np.copy(y_des[:, -1]).astype(np.float)
    ...
    def gen_psi(self, x):
        ...
        if isinstance(x, np.ndarray):
            x = x[:, None]
        return np.exp(-self.variance * (x - self.center)**2)
